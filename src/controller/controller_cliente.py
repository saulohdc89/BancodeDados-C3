...
class Controller_Cliente:
    ...
    def inserir_cliente(self) -> Clientes:
        ...
        if self.verifica_existencia_cliente(cpf):
            # Solicita ao usuario o novo nome/endereço/telefone
            nome = input("Nome (Novo): ")
            endereco = input("Endereco (Novo): ")
            telefone = input("Telefone (Novo): ")
            # Insere e persiste o novo cliente
            self.mongo.db["clientes"].insert_one({"cpf": cpf, "nome": nome,"endereco":endereco,"telefone":telefone})
    ...
    def atualizar_cliente(self) -> Clientes:
        # Cria uma nova conexão com o banco que permite alteração
        self.mongo.connect()
        ...
        if not self.verifica_existencia_cliente(cpf):
            ...
            self.mongo.db["clientes"].update_one({"cpf": f"{cpf}"}, {"$set": {"nome": novo_nome,"endereco":novo_endereco, "telefone":novo_telefone}})
            # Recupera os dados do novo cliente criado transformando em um DataFrame
            df_cliente = self.recupera_cliente(cpf)
            # Cria um novo objeto cliente
            cliente_atualizado = Clientes(df_cliente.cpf.values[0], df_cliente.nome.values[0],df_cliente.endereco.values[0],df_cliente.telefone.values[0])
            # Exibe os atributos do novo cliente
    ...
    def verifica_existencia_cliente(self, cpf:str=None, external:bool=False) -> bool:
        ...
        # Recupera os dados do novo cliente criado transformando em um DataFrame
        df_cliente = pd.DataFrame(self.mongo.db["clientes"].find({"cpf":f"{cpf}"}, {"cpf": 1, "nome": 1,"endereco":1,"telefone":1, "_id": 0}))
